...
$script_name = "libjpeg-turbo-1.3.1-x64.ps1"
...
$target_dir = "$root_path\$($target_nam)-x64"
...
LogBegin "Building libjpeg-turbo (x64)"
...
cmake -G "`"$target_cmake_gen_x64`""
Invoke-Expression -Command "msbuild.exe $target_msbuild_opt_x64 >> $script_log 2>&1"
